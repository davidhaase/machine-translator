...
# def save_clean_data(sentences, filename):
#     # dump(sentences, open(filename, 'wb'))
#     s3.dump(sentences, filename)
#     print('Saved: %s' % filename)
#
# # load dataset
# filename = 'LanguageTexts/deu.txt'
...
def evaluate_model(model, tokenizer, sources, raw_dataset):
	...
	for i, source in enumerate(sources):
		...
		if i < 10:
			print('src={}, target={}, predicted={}'.format(source, raw_dataset, translation))
...
# load datasets
dataset = load_clean_sentences('pickles/de2en_sentence_pairs.pkl')
train = load_clean_sentences('pickles/de2en_train.pkl')
test = load_clean_sentences('pickles/de2en_test.pkl')
# prepare english tokenizer
eng_tokenizer = create_tokenizer(dataset[:, 0])
eng_vocab_size = len(eng_tokenizer.word_index) + 1
eng_length = max_length(dataset[:, 0])
...
train_X = encode_sequences(ger_tokenizer, ger_length, 'Ich bin ein Berliner')
# load model
model = load_model('models/model.h5')
evaluate_model(model, eng_tokenizer, train_X, 'I am from Berlin')
